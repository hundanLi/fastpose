...
"""
Example of python frontend for backend.py
...
def start(file_path, port):
    ...
    while True:
        ...
        tmp_time = time.time()
        r = requests.post("http://localhost:" + port, headers={'Content-Type': 'application/octet-stream'},
                          data=frame_data)
        fps = int(1 / (time.time() - tmp_time))

        json_pose = json.loads(r.text)
        poses = [Pose2D.build_from_JSON(p['pose_2d']) for p in json_pose]

        ids = [p['id'] for p in json_pose]
